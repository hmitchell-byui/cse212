...
public static class Algorithms
{
    ...
    //With only one erference for size in the forlopp, this algoithm is O(n) ^^^
    ...
    //With two references for size in the forloop, this algorithm is O(n) * O(n) = O(n^2) ^^^
...
//With the size being halved each time, this algorithm is O(log n) ^^^